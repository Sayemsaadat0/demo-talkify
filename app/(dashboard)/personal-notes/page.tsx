...
import { NOTE_DELETE_API } from '@/api/api'

export default function PersonalNotesPage() {
  const [page, setPage] = useState(1)
  const debouncedPage = useDebounce(page, 500) // wait 0.5s before fetching
  const { data, loading, refetch } = useGetNote(debouncedPage)
  ...
  const handleDelete = useCallback(async (id: number) => {
    ...
    try {
      const response = await fetch(`${NOTE_DELETE_API}${id}`, {
        method: "DELETE",
        headers: {
  ...
  return (
    <div className="space-y-6 p-3">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        {loading ? (
          <div className="p-4 text-gray-500">Loading notes...</div>
        ) : (
          <>
                  ...
                  {data?.data?.data?.length ? (
                    data.data.data.map((note) => (
                      <tr key={note.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-2 border">{note.property_id}</td>
